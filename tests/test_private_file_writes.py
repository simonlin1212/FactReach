...
def test_safe_install_with_proxy_makes_no_persistent_writes(
    ...
    assert not (isolated_home / ".openclaw" / "skills" / "factreach").exists()
    assert not (isolated_home / ".claude" / "skills" / "factreach").exists()
    assert not (isolated_home / ".agents" / "skills" / "factreach").exists()
    output = capsys.readouterr().out
...
def test_install_system_flag_explicitly_enables_writes(
    ...
    monkeypatch.setattr(
        cli.sys,
        "argv",
        ["factreach", "install", "--env", "local", "--system"],
    )
