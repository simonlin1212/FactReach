...
class TestCLI:
    def test_version(self, capsys):
        with pytest.raises(SystemExit) as exc_info:
            with patch("sys.argv", ["factreach", "version"]):
                main()
        assert exc_info.value.code == 0
        captured = capsys.readouterr()
        assert "FactReach v1.0.0" in captured.out

    def test_no_command_shows_help(self, capsys):
        with pytest.raises(SystemExit) as exc_info:
            with patch("sys.argv", ["factreach"]):
                main()
        assert exc_info.value.code == 0
    ...
    def test_doctor_runs(self, capsys):
        with patch(
            ...
            return_value="FactReach\n✅ 网页可用",
        ), patch("sys.argv", ["factreach", "doctor"]):
            main()
        captured = capsys.readouterr()
        assert "FactReach" in captured.out
        assert "✅" in captured.out

    def test_doctor_is_read_only_and_never_installs_skill(
        self, monkeypatch, tmp_path, capsys
    ):
        skill_dir = tmp_path / ".agents" / "skills" / "factreach"
        skill_dir.mkdir(parents=True)
        skill_file = skill_dir / "SKILL.md"
        custom_content = "# custom FactReach skill\n"
    ...
    def test_transcribe_command_prints_text(self, capsys):
        with patch("agent_reach.transcribe.transcribe", return_value="hello transcript"):
            with patch("sys.argv", ["factreach", "transcribe", "audio.mp3"]):
                main()
        captured = capsys.readouterr()
    ...
    def test_transcribe_command_writes_output_file(self, capsys, tmp_path):
        ...
        with patch("agent_reach.transcribe.transcribe", return_value="saved text"):
            with patch("sys.argv", ["factreach", "transcribe", "audio.mp3", "-o", str(out_file)]):
                main()
    ...
    def test_douyin_resolve_command_prints_json(self, capsys):
        resolved = {
            "video_id": "123",
        ...
        with patch(
            "agent_reach.channels.douyin.DouyinChannel.resolve",
            return_value=resolved,
        ) as mock_resolve, patch(
            "sys.argv",
            ["factreach", "douyin", "resolve", "share https://v.douyin.com/abc/"],
        ):
            main()

        assert cli.json.loads(capsys.readouterr().out) == resolved
        mock_resolve.assert_called_once_with("share https://v.douyin.com/abc/")

    def test_douyin_transcribe_requires_explicit_flag_and_routes_provider(
        self, capsys
    ):
        resolved = {
            "video_id": "123",
        ...
        with patch(
            "agent_reach.channels.douyin.DouyinChannel.transcribe",
            return_value=resolved,
    ...
    def test_web_read_command_routes_explicit_browser(self, capsys):
        with patch(
            "agent_reach.channels.web.WebChannel.read",
            return_value="rendered page",
        ) as mock_read, patch(
            "sys.argv",
            [
                "factreach",
        ...
        assert capsys.readouterr().out.strip() == "rendered page"
    ...
    def test_web_read_output_failure_is_reported_without_traceback(
        self, tmp_path, capsys
    ):
        missing_parent = tmp_path / "missing" / "page.md"
        with patch(
            "agent_reach.channels.web.WebChannel.read",
            return_value="rendered page",
        ):
    ...
    def test_firecrawl_key_configure_from_stdin(self, monkeypatch, tmp_path, capsys):
        config_dir = tmp_path / ".agent-reach"
        monkeypatch.setattr(Config, "CONFIG_DIR", config_dir)
        monkeypatch.setattr(Config, "CONFIG_FILE", config_dir / "config.yaml")
        monkeypatch.setattr("sys.stdin.read", lambda _limit: "fc-secret\n")

        with patch(
            "sys.argv",
            ["factreach", "configure", "firecrawl-key", "--stdin"],
        ):
            main()

        assert Config().get("firecrawl_api_key") == "fc-secret"
        assert "fc-secret" not in capsys.readouterr().out

    def test_firecrawl_url_configure(self, monkeypatch, tmp_path):
        config_dir = tmp_path / ".agent-reach"
        monkeypatch.setattr(Config, "CONFIG_DIR", config_dir)
        monkeypatch.setattr(Config, "CONFIG_FILE", config_dir / "config.yaml")
        ...
        assert Config().get("firecrawl_api_url") == "http://localhost:3002/v2"
    ...
    def test_install_weibo_routes_to_opencli_once(self, monkeypatch, capsys):
        calls = []
        ...
        monkeypatch.setattr(cli, "_install_mcporter", lambda: None)
        monkeypatch.setattr(
            cli, "_install_weibo_deps", lambda: calls.append("weibo")
        )
        monkeypatch.setattr(cli, "_install_skill", lambda: None)
        monkeypatch.setattr("agent_reach.doctor.check_all", lambda config: {})
        monkeypatch.setattr("agent_reach.doctor.format_report", lambda results: "report")
    ...
    def test_install_browser_harness_uses_official_git_source(
        self, monkeypatch, capsys
    ):
        from agent_reach.backends.browser_harness import BrowserHarnessStatus

        statuses = iter(
            [
                BrowserHarnessStatus("off", False, "missing"),
                BrowserHarnessStatus("warn", True, "installed"),
            ]
        )
        calls = []
        monkeypatch.setattr(
            "agent_reach.backends.browser_harness.browser_harness_status",
        ...
        assert cli._install_browser_harness_deps() is True
        assert calls == [
            [
                "/usr/local/bin/uv",
                "tool",
        ...
        assert "不会自动连接 Chrome" in capsys.readouterr().out

    def test_install_browser_harness_rejects_broken_post_install_runtime(
        self, monkeypatch, capsys
    ):
        from agent_reach.backends.browser_harness import BrowserHarnessStatus
        ...
        statuses = iter(
            [
                BrowserHarnessStatus("off", False, "missing"),
                BrowserHarnessStatus("error", True, "broken shim"),
            ]
        )
        monkeypatch.setattr(
            "agent_reach.backends.browser_harness.browser_harness_status",
        ...
        assert cli._install_browser_harness_deps() is False
    ...
    def test_install_server_dry_run_skips_opencli_only_channels(self, monkeypatch, capsys):
        ...
        cli._cmd_install(
            Namespace(
                ...
                safe=False,
                dry_run=True,
                channels="facebook,instagram,weibo,opencli,browser-harness,bilibili",
            )
        )

        out = capsys.readouterr().out
        assert (
            "服务器环境跳过：browser-harness, facebook, instagram, opencli, weibo"
            in out
        )
