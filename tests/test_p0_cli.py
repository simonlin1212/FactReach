...
def test_configure_reads_secret_from_stdin_without_echoing_it(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["factreach", "configure", "groq-key", "--stdin"],
    )
...
def test_configure_uses_hidden_prompt_when_no_value_is_given(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["factreach", "configure", "openai-key"],
    )
...
def test_setup_uses_hidden_prompts_for_secrets(monkeypatch, capsys):
    ...
    prompts = []
    config = _MemoryConfig()
    config.config_path = Path("/tmp/factreach-test-config.yaml")
...
def test_configure_positional_secret_warns_to_use_safe_input(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["factreach", "configure", "groq-key", "legacy-secret"],
    )
...
def test_configure_missing_value_exits_one(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["factreach", "configure", "github-token"],
    )
...
def test_twitter_cookie_parse_failure_exits_one(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["factreach", "configure", "twitter-cookies", "not-a-cookie"],
    )
...
def test_uninstall_preserves_shared_legacy_config_by_default(
    isolated_home, monkeypatch, capsys
):
    config_dir = isolated_home / ".agent-reach"
    config_dir.mkdir()
    config_file = config_dir / "config.yaml"
    config_file.write_text("twitter_cookies: secret\n", encoding="utf-8")
    monkeypatch.setattr("shutil.which", lambda _name: None)

    cli._cmd_uninstall(
        Namespace(dry_run=False, keep_config=False, purge_config=False)
    )

    assert config_file.exists()
    output = capsys.readouterr().out
    assert "Preserving shared compatibility config" in output
    assert "--purge-config" in output


def test_uninstall_purges_shared_config_only_with_explicit_flag(
    isolated_home, monkeypatch
):
    config_dir = isolated_home / ".agent-reach"
    config_dir.mkdir()
    (config_dir / "config.yaml").write_text("token: secret\n", encoding="utf-8")
    monkeypatch.setattr("shutil.which", lambda _name: None)

    cli._cmd_uninstall(
        Namespace(dry_run=False, keep_config=False, purge_config=True)
    )
...
def test_uninstall_preserves_mcporter_entries_without_agent_reach_provenance(
    ...
    """Names and endpoints alone do not prove FactReach owns an MCP entry."""
    calls = []
    payload = {
        "servers": [
            {
