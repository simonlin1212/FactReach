...
def test_localized_readmes_keep_current_bilibili_and_xhs_routes():
    ...
    for path in readmes:
        ...
        for backend in ("OpenCLI", "xiaohongshu-mcp", "xhs-cli"):
            assert backend in text, path.relative_to(ROOT)


def test_english_readme_matches_shipped_channels():
    """The maintained English README must describe the current registry."""
    text = (ROOT / "README_en.md").read_text(encoding="utf-8").lower()
    assert "23 internet channels" in text
    assert "douyin" in text
    assert "weibo" in text


def test_public_guidance_never_installs_the_unrelated_pypi_package():
    """Until a verified PyPI release exists, public installs must use GitHub."""
    candidates = _policy_documents() + [
        ROOT / "agent_reach" / "integrations" / "mcp_server.py",
    ]
    bare_install = re.compile(
        r"\bpip\s+install(?:\s+--upgrade)?\s+['\"]?factreach(?:\[[^\]]+\])?\b",
        re.IGNORECASE,
    )
    violations = []
    for path in candidates:
        for line_number, line in enumerate(
            ...
            if bare_install.search(line) and (
                "github.com/simonlin1212/FactReach" not in line
            ):
                violations.append(
                    f"{path.relative_to(ROOT)}:{line_number}: {line.strip()}"
                )
...
def test_public_guidance_never_puts_secrets_in_process_arguments():
    ...
    forbidden = (
        ...
        "factreach configure xhs-cookies '",
        "factreach configure groq-key gsk_",
        "factreach configure openai-key sk-",
        "factreach configure github-token gh",
        "factreach configure proxy http",
    )
    violations = []
    for path in _policy_documents():
        text = path.read_text(encoding="utf-8")
...
def test_video_reference_has_content_level_youtube_fallbacks():
    ...
    assert "factreach transcribe" in text
...
def test_skill_routes_finance_and_documents_opencli_discovery():
    ...
    assert "factreach configure --from-browser chrome --platform xueqiu" in text
