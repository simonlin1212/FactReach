# -*- coding: utf-8 -*-
"""Tests for 'factreach skill' command and _install_skill / _uninstall_skill."""
...
class TestSkillCommand(unittest.TestCase):
    ...
    def test_english_readme_does_not_restore_retired_linkedin_server(self):
        root = Path(__file__).resolve().parents[1]
        content = (root / "README_en.md").read_text(encoding="utf-8")
            ...
            for dirpath, _, filenames in os.walk(tmpdir):
                if "SKILL.md" in filenames:
                    ...
                    with open(os.path.join(dirpath, "SKILL.md"), encoding="utf-8") as f:
                        content = f.read()
                    self.assertIn("FactReach", content)
    ...
    def test_uninstall_skill_removes_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            skill_path = os.path.join(tmpdir, ".openclaw", "skills", "factreach")
    ...
    def test_install_creates_dir_if_parent_exists(self):
        """_install_skill should create factreach dir inside existing skill dir."""
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create the .openclaw/skills parent but not factreach subdir
            ...
            target = os.path.join(skill_parent, "factreach", "SKILL.md")
    ...
    def test_install_uses_english_skill_for_english_locale(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                os.path.exists(os.path.join(skill_parent, "factreach", "references"))
            )

    def test_install_keeps_legacy_chinese_default_for_non_language_locales(self):
        """C/POSIX/unknown locales historically installed the Chinese skill."""
        for locale in ("", "C.UTF-8", "ja_JP.UTF-8"):
            with self.subTest(locale=locale), tempfile.TemporaryDirectory() as tmpdir:
                skill_parent = os.path.join(tmpdir, ".openclaw", "skills")
                os.makedirs(skill_parent)
                with patch(
                    "agent_reach.cli.os.path.expanduser",
                    ...
                    env = {"LANG": locale} if locale else {}
                    with patch.dict(os.environ, env, clear=True):
                        _install_skill()
    ...
    def test_install_explicit_language_override_wins(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            skill_parent = os.path.join(tmpdir, ".openclaw", "skills")
            os.makedirs(skill_parent)
            with patch(
                ...
                os.environ,
                {"AGENT_REACH_LANG": "en", "LANG": "zh_CN.UTF-8"},
                clear=True,
            ):
                _install_skill()
