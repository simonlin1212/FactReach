...
# Try env var first, then factreach config.yaml
...
GROQ_API_KEY="${GROQ_API_KEY:?请设置 GROQ_API_KEY 环境变量或运行 factreach configure groq-key}"
...
if ! WORK_DIR=$(mktemp -d "${TEMP_ROOT%/}/factreach-xiaoyuzhou.XXXXXX"); then
    echo "❌ 无法创建临时目录" >&2
    exit 1
fi
...
for i in $(seq 0 $((NUM_CHUNKS - 1))); do
    ...
    if [ "$HTTP_CODE" != "200" ]; then
        ...
        if [ "$HTTP_CODE" = "429" ]; then
            ...
            case "$HTTP_CODE" in
                5??)
                    echo "   可尝试兜底：factreach transcribe \"$AUDIO_URL\""
                    ;;
            esac
            exit 1
        fi
...
def call_groq(text):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "factreach-xiaoyuzhou/1.0",
        },
    )
...
if [ -z "$OUTPUT" ]; then
    if ! OUTPUT=$(mktemp "${TEMP_ROOT%/}/factreach-transcript.XXXXXX"); then
        echo "❌ 无法安全创建输出文件" >&2
        exit 1
    fi
fi
