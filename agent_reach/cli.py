# -*- coding: utf-8 -*-
"""
FactReach CLI — installer, doctor, and configuration tool.

Usage:
    factreach install --env=auto
    factreach doctor
    factreach configure twitter-cookies
...
def _preferred_language(*, default: str = "en") -> str:
    """Resolve a supported language while preserving each caller's default."""
    explicit = (
        os.environ.get("FACTREACH_LANG", "")
        or os.environ.get("AGENT_REACH_LANG", "")
    ).strip().lower()
    if explicit:
        return "zh" if explicit.startswith(("zh", "chinese")) else "en"

    for key in ("LC_ALL", "LC_MESSAGES", "LANG"):
        locale = os.environ.get(key, "").strip().lower()
        if locale:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="factreach",
        description="Evidence-first search and reading across 23 internet channels",
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Show debug logs")
    parser.add_argument("--version", action="version", version=f"FactReach v{__version__}")
    ...
    p_install.add_argument("--channels", default="",
                           help="Comma-separated optional channels to install "
                                ...
                                "douyin,weibo,firecrawl,browser-harness,"
                                "open-websearch,hackernews,wechat,academic,"
                                "stackoverflow,github-trending,all)")
    ...
    p_uninstall = sub.add_parser(
        "uninstall",
        help="Remove FactReach skill files; preserve shared legacy config by default",
    )
    ...
    p_uninstall.add_argument("--keep-config", action="store_true",
                             help="Deprecated compatibility flag; config is preserved by default")
    p_uninstall.add_argument(
        "--purge-config",
        action="store_true",
        help="Also delete shared ~/.agent-reach/ config, tokens, cookies, and tools",
    )
    ...
    # ── evidence-first system policy ──
    p_policy = sub.add_parser(
        "policy",
        help="Manage search-before-reasoning rules in CLAUDE.md / AGENTS.md",
    )
    p_policy_group = p_policy.add_mutually_exclusive_group(required=True)
    p_policy_group.add_argument(
        "--show",
        action="store_true",
        help="Print the policy and recommended instruction-file locations",
    )
    p_policy_group.add_argument(
        "--check",
        action="store_true",
        help="Check whether the managed policy is installed",
    )
    p_policy_group.add_argument(
        "--install",
        action="store_true",
        help="Explicitly append or update the managed policy block",
    )
    p_policy_group.add_argument(
        "--uninstall",
        action="store_true",
        help="Remove only the managed policy block",
    )
    p_policy.add_argument(
        "--target",
        choices=["auto", "claude", "codex", "both"],
        default="auto",
        help="Instruction target (install/uninstall requires claude, codex, or both)",
    )
    p_policy.add_argument(
        "--scope",
        choices=["user", "project"],
        default="user",
        help="User-wide config or current-project config (default: user)",
    )
    p_policy.add_argument(
        "--lang",
        choices=["auto", "zh", "en"],
        default="auto",
        help="Policy language (default: infer from locale)",
    )
    ...
    # ── douyin ──
    p_douyin = sub.add_parser(
        "douyin",
        help="Resolve one Douyin share link into copy and a playback URL",
    )
    douyin_sub = p_douyin.add_subparsers(dest="douyin_command", required=True)
    p_douyin_resolve = douyin_sub.add_parser("resolve", help="Resolve one share link")
    p_douyin_resolve.add_argument("share_text", help="Share text or Douyin URL")
    p_douyin_resolve.add_argument(
        "--transcribe",
        action="store_true",
        help="Also transcribe spoken audio via configured Groq/OpenAI Whisper",
    )
    p_douyin_resolve.add_argument(
        "--provider",
        choices=["auto", "groq", "openai"],
        default="auto",
        help="Transcription provider when --transcribe is set",
    )
    p_douyin_resolve.add_argument(
        "--allow-provider-fallback",
        action="store_true",
        help="Allow auto transcription to try another configured provider",
    )

    # ── web ──
    p_web = sub.add_parser("web", help="Read one public webpage")
    web_sub = p_web.add_subparsers(dest="web_command", required=True)
    p_web_read = web_sub.add_parser("read", help="Read one webpage")
    p_web_read.add_argument("url", help="Public HTTP(S) URL")
    p_web_read.add_argument(
        "--backend",
        choices=["auto", "jina", "firecrawl", "browser-harness"],
        default="auto",
        help="Backend order or explicit backend",
    )
    p_web_read.add_argument(
        "--allow-browser",
        action="store_true",
        help="Allow auto mode to open the real Chrome after non-browser backends fail",
    )
    p_web_read.add_argument(
        "-o",
        "--output",
        default=None,
        help="Write page text to a UTF-8 file instead of stdout",
    )
    ...
    if args.command == "configure" and args.from_browser:
        ...
        if args.platform in manual_keys:
            p_conf.error(
                ...
                f"`factreach configure {manual_keys[args.platform]} ...`"
            )
    ...
    if (
        ...
        p_douyin_resolve.error(
            "--allow-provider-fallback requires --provider auto"
        )
    if (
        args.command == "policy"
        and (args.install or args.uninstall)
        and args.target == "auto"
    ):
        p_policy.error("--install/--uninstall requires --target claude, codex, or both")
    ...
    if args.command == "version":
        print(f"FactReach v{__version__}")
        sys.exit(0)
...
def _cmd_install(args):
    ...
    CHANNEL_INSTALLERS = {
        ...
        "douyin":      _install_douyin_deps,
        "firecrawl":   _install_firecrawl_deps,
        "browser-harness": _install_browser_harness_deps,
        "opencli":     _install_opencli_deps,  # cross-channel backend, desktop only
        "open-websearch": _install_open_websearch_deps,
        "hackernews":  _install_opencli_public_deps,
        "wechat":      _install_wechat_deps,
        "academic":    _install_opencli_public_deps,
        "stackoverflow": _install_opencli_public_deps,
        "github-trending": _install_opencli_public_deps,
    ...
    if unknown_channels:
        ...
        print(
            f"factreach install: error: unknown channel(s): {unknown}. "
    ...
    if requested_channels and not dry_run and not safe_mode:
        ...
        for ch_name in sorted(requested_channels):
            # The Weibo installer verifies its adapter and includes the generic
            # OpenCLI runtime setup, so an explicit ``opencli`` alias is redundant.
            if ch_name == "opencli" and "weibo" in requested_channels:
                continue
    ...
    if env == "local" and needs_cookies and not dry_run:
        ...
        for channel in sorted(requested_channels & COOKIE_CHANNELS):
            if channel == "twitter":
                print("  factreach configure twitter-cookies")
            elif channel == "xiaohongshu":
                print("  factreach configure xhs-cookies")
            else:
                print(
                    "  factreach configure --from-browser chrome "
                    f"--platform {channel}"
                )
    ...
    if env == "server":
        ...
        print("   保存代理供 Agent 使用：factreach configure proxy（隐藏输入）")
    ...
    if not dry_run:
        ...
        if safe_mode:
            ...
            if not requested_channels:
                ...
                print("   factreach install --system --channels=twitter,xiaohongshu,reddit,facebook,instagram,...")
                print("   factreach install --system --channels=all  (install everything)")

            print()
            print("Evidence-first system rule is not installed silently.")
            print("   Preview: factreach policy --show")
            print("   Check:   factreach policy --check")
            print("   Install after approval: factreach policy --install --target both --scope user")

            # Star reminder
            print()
            print("如果 FactReach 帮到了你，给个 Star 让更多人发现它吧：")
            print("   https://github.com/simonlin1212/FactReach")
...
def _install_skill(force: bool = True):
    """Install FactReach as an agent skill for supported agent clients."""
    ...
    def _skill_resource_name() -> str:
        return (
            "SKILL.md"
            if _preferred_language(default="zh") == "zh"
            else "SKILL_en.md"
        )
    ...
    for skill_dir, platform_name in skill_dirs:
        if os.path.isdir(skill_dir):
            target = os.path.join(skill_dir, "factreach")
    ...
    if not installed:
        ...
        target = os.path.expanduser("~/.agents/skills/factreach")
...
def _uninstall_skill():
    ...
    skill_dirs = [
        ("~/.config/opencode/skills/factreach", "OpenCode"),
        ("~/.openclaw/skills/factreach", "OpenClaw"),
        ("~/.claude/skills/factreach", "Claude Code"),
        ("~/.agents/skills/factreach", "Agent"),
    ]
    ...
    if openclaw_home:
        skill_dirs.insert(
            0,
            (os.path.join(openclaw_home, ".openclaw", "skills", "factreach"), "OpenClaw"),
        )
...
def _cmd_policy(args):
    """Show, check, or explicitly install the evidence-first system policy."""
    ...
    if args.show:
        print("This rule belongs in the agent instruction layer, not only in SKILL.md.")
        print("User-wide: ~/.claude/CLAUDE.md and/or ~/.codex/AGENTS.md")
        print("Project-only: ./CLAUDE.md and/or ./AGENTS.md")
        print()
        print(load_policy(language), end="")
        return

    try:
        targets = resolve_targets(args.target, args.scope)
    except SystemPolicyError as exc:
        print(f"policy target resolution failed: {exc}")
    ...
    if args.check:
        ...
        for name, path in targets.items():
            ...
            all_installed = all_installed and status == "installed"
    ...
    for name, path in targets.items():
        try:
            if args.install:
                result = install_policy(path, language=language)
            else:
                result = remove_policy(path)
            print(f"{name}: {result} — {path}")
        except (OSError, SystemPolicyError) as exc:
            failures.append(name)
            print(f"{name}: failed — {path}: {exc}")
    if failures:
        raise SystemExit(1)
...
def _install_xiaoyuzhou_deps():
    ...
    if has_key:
        ...
        print("     Then run: factreach configure groq-key（隐藏输入）")
...
def _install_opencli_public_deps():
    """Install OpenCLI and verify the adapters used by public-source channels.

    This path verifies only the public HTTP adapters. It deliberately does not
    require a browser-backed adapter or ask for a Chrome extension.
    """
    import shutil
    import subprocess
    ...
    print("Setting up OpenCLI public-source adapters (no login required)...")
    probe = probe_opencli_public_adapters()
    if probe.ok:
        print("  ✅ OpenCLI public-source adapters already installed")
        return True
    ...
    if (
        ...
        and probe_opencli_public_adapters().ok
    ):
        print("  ✅ OpenCLI public-source adapters installed")
        return True
    print(f"  [!]  OpenCLI install failed. Run: npm install -g {OPENCLI_PACKAGE}")
    return False
...
def _install_wechat_deps():
    """Verify the WeChat adapter, then set up its Browser Bridge runtime."""
    if not _install_opencli_wechat_deps():
        return False
    return _install_opencli_deps()


def _install_opencli_wechat_deps():
    """Install OpenCLI and verify the browser-backed WeChat adapter."""
    import shutil
    import subprocess
    ...
    print("Setting up OpenCLI WeChat adapter (Chrome login required)...")
    probe = probe_opencli_wechat_adapter()
    if probe.ok:
        print("  ✅ OpenCLI WeChat adapter already installed")
        return True
    ...
    if (
        result is not None
        and result.returncode == 0
        and probe_opencli_wechat_adapter().ok
    ):
        print("  ✅ OpenCLI WeChat adapter installed")
        return True
    print(
        "  [!]  OpenCLI WeChat adapter unavailable. Run: "
...
def _install_weibo_deps():
    """Install OpenCLI, require its Weibo adapter, then explain Chrome setup."""
    import shutil
    import subprocess
    ...
    print("Setting up OpenCLI Weibo adapter (Chrome login required)...")
    probe = probe_opencli_weibo_adapter()
    if not probe.ok:
        npm_cmd = shutil.which("npm")
        if not npm_cmd:
            print("  [!]  OpenCLI requires Node.js >= 20. Install Node first:")
        ...
        if (
            ...
            or not probe_opencli_weibo_adapter().ok
        ):
            print(
                "  [!]  OpenCLI Weibo adapter unavailable. Run: "
                f"npm install -g {OPENCLI_PACKAGE}"
            )
            return False
        print("  ✅ OpenCLI Weibo adapter installed")
    else:
        print("  ✅ OpenCLI Weibo adapter already installed")
    return _install_opencli_deps()


def _install_open_websearch_deps():
    """Install the official open-websearch CLI for zero-auth web search."""
    import shutil
    import subprocess

    from agent_reach.channels.web_search import open_websearch_runtime_ready

    print("Setting up open-websearch (zero-auth web search)...")
    executable = shutil.which("open-websearch")
    if executable and open_websearch_runtime_ready(executable):
        print("  ✅ open-websearch already installed")
        return True
    npm_cmd = shutil.which("npm")
    if not npm_cmd:
        print("  [!]  open-websearch requires Node.js. Install Node first:")
        print("       https://nodejs.org  （或 brew install node）")
        return False
    try:
        result = subprocess.run(
            [npm_cmd, "install", "-g", "open-websearch"],
            capture_output=True,
            encoding="utf-8",
            errors="replace",
            timeout=300,
    ...
    if (
        result is not None
        and result.returncode == 0
        and (installed := shutil.which("open-websearch"))
        and open_websearch_runtime_ready(installed)
    ):
        print("  ✅ open-websearch installed (DuckDuckGo-first fallback)")
        return True
    print("  [!]  open-websearch install failed. Run: npm install -g open-websearch")
    return False
...
def _install_douyin_deps():
    """Report the built-in Douyin resolver and optional ASR dependency."""
    import shutil

    print("Setting up Douyin single-link resolver...")
    print("  ✅ 分享链接、发布文案、作者和播放地址解析已内置，无需登录")
    if shutil.which("ffmpeg"):
        print("  ✅ ffmpeg available for optional spoken-word transcription")
    else:
        print("  -- 转写为可选能力；需要时安装 ffmpeg 并配置 Groq/OpenAI")
    return True


def _install_firecrawl_deps():
    """Explain Firecrawl configuration; no SDK install is required."""
    from agent_reach.backends.firecrawl import firecrawl_status

    print("Setting up Firecrawl dynamic-page backend...")
    status = firecrawl_status()
    if status.configured:
        print(f"  ✅ {status.hint}")
    else:
        print(f"  -- {status.hint}")
        print("     自托管可用：factreach configure firecrawl-url <HTTPS_URL>")
    print("  ✅ 使用内置 HTTP 客户端，无需额外 Python SDK")
    return True


def _install_browser_harness_deps():
    """Install browser-harness without attaching to or inspecting Chrome."""
    import shutil
    import subprocess
    ...
    print("Setting up browser-harness (explicit real-Chrome backend)...")
    status = browser_harness_status()
    if status.installed and status.status != "error":
        print("  ✅ browser-harness already installed")
        print("  -- 安装/诊断不会自动连接 Chrome 或读取标签页")
        return True

    for tool, install_args in (
        ("uv", ["tool", "install", BROWSER_HARNESS_PACKAGE]),
        ("pipx", ["install", BROWSER_HARNESS_PACKAGE]),
    ):
        ...
        try:
            ...
            continue
        post_install_status = browser_harness_status()
        if (
            result.returncode == 0
            and post_install_status.installed
            and post_install_status.status != "error"
        ):
            print("  ✅ browser-harness installed")
            print("  -- 安装/诊断不会自动连接 Chrome 或读取标签页")
            print("  -- 首次使用前请在 Chrome 开启远程调试")
            return True

    print(
        "  [!]  browser-harness install failed. Run: uv tool install "
        + BROWSER_HARNESS_PACKAGE
    )
    return False
...
def _cmd_configure(args):
    ...
    if args.from_browser:
        ...
        print()
        if found_any:
            print("✅ Cookies configured! Run `factreach doctor` to see updated status.")
        else:
    ...
    # ── Manual configure ──
    if not args.key:
        print("Usage: factreach configure <key> [--stdin]")
        print("   Omit the value to enter it through a hidden prompt.")
        print(
            "   or: factreach configure --from-browser chrome "
            "--platform xueqiu"
        )
        return
    ...
    if args.key == "proxy":
        ...
        print("✅ Firecrawl key configured!")
        ...
        if (
            ...
            print(
                "[X] Firecrawl URL must be HTTPS, except loopback self-hosts."
            )
            raise SystemExit(1)
        config.set("firecrawl_api_url", value.rstrip("/"))
        print("✅ Firecrawl API URL configured!")
...
def _cmd_douyin(args):
    """Resolve one Douyin share link, optionally adding ASR text."""
    ...
    try:
        if args.transcribe:
            result = channel.transcribe(
                args.share_text,
                provider=args.provider,
                allow_provider_fallback=args.allow_provider_fallback,
            )
        else:
            result = channel.resolve(args.share_text)
    except (DouyinResolveError, TranscribeError) as exc:
        print(f"❌ {scrub_url_credentials(exc)}", file=sys.stderr)
        raise SystemExit(1) from None
    print(json.dumps(result, ensure_ascii=False, indent=2))


def _cmd_web(args):
    """Read one page through the explicitly selected fallback policy."""
    from pathlib import Path
    ...
    try:
        ...
        raise SystemExit(1) from None
    if args.output:
        print(f"✅ Page content written to {args.output}")
    else:
        print(content)
...
def _configure_xhs_cookies(value) -> bool:
    ...
    if not value:
        ...
        print("   Run `factreach configure xhs-cookies` and paste the Cookie-Editor export.")
        print("   For automation, pass the same value through --stdin.")
        return False
...
def _cmd_uninstall(args):
    """Remove FactReach-owned files while preserving shared config by default."""
    import shutil
    import subprocess
    ...
    if purge_config:
        ...
        print(
            "  Preserving shared compatibility config by default: "
            f"{config_dir}"
        )
        print("  Use --purge-config only after confirming no Agent-Reach tools need it.")
        ...
        try:
            ...
            for mcp_name in ("exa", "xiaohongshu"):
                ...
                print(
                    ...
                    "FactReach 管理；已保留。若确认不再需要，请手动移除。"
                )
    ...
    if dry_run:
        ...
        if removed_any:
            print("FactReach data removed.")
        elif mcporter_cleanup_skipped:
            print("No proven FactReach-managed mcporter data was removed.")
        else:
            print("Nothing to remove — already clean.")

    print()
    print("Optional: remove the FactReach Python package itself:")
    print("  pip uninstall factreach")
    print()
    print("Optional: remove tools installed by FactReach:")
...
def _cmd_setup():
    ...
    print("运行 factreach doctor 查看完整状态")
    print()
...
_UPDATE_INSTRUCTIONS = (
    ...
    "  帮我更新 FactReach：https://raw.githubusercontent.com/simonlin1212/FactReach/main/docs/update.md\n"
    "仅更新本体（不含上游工具和 skill）：\n"
    "  pip install --upgrade https://github.com/simonlin1212/FactReach/archive/main.zip"
)
...
def _cmd_check_update():
    ...
    release_url = "https://api.github.com/repos/simonlin1212/FactReach/releases/latest"
    commit_url = "https://api.github.com/repos/simonlin1212/FactReach/commits/main"
...
def _cmd_watch():
    ...
    resp, err, _attempts = _github_get_with_retry(
        "https://api.github.com/repos/simonlin1212/FactReach/releases/latest",
        timeout=10,
        retries=2,
    )
    ...
    if not issues and not update_available:
        print(f"FactReach: 全部正常 ({ok}/{total} 渠道可用，v{__version__} 已是最新)")
        return

    print("FactReach 监控报告")
    print("=" * 40)
    ...
    if update_available:
        ...
        print("    帮我更新 FactReach：https://raw.githubusercontent.com/simonlin1212/FactReach/main/docs/update.md")
