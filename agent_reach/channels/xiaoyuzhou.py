...
class XiaoyuzhouChannel(Channel):
    ...
    def check(self, config=None):
        ...
        if not os.path.isfile(script):
            return "off", (
                "转录脚本未安装。运行：\n"
                "  factreach install --env=auto --system --channels=xiaoyuzhou\n"
                "  或手动复制 transcribe.sh 到 ~/.agent-reach/tools/xiaoyuzhou/"
            )

        # Check GROQ_API_KEY — prefer env var, fall back to FactReach config
        ...
        if not has_key:
            return "warn", (
                ...
                "  2. 运行: factreach configure groq-key（隐藏输入）"
            )
