...
`~/.agent-reach/config.yaml`，供 `factreach doctor` 检查显式凭据是否齐全。
...
```bash
factreach configure twitter-cookies --sync-legacy-twitter
```

`factreach uninstall` 只会提醒这些 legacy 副本，不会自动删除。需要清理时，
先让用户确认，再手工删除上述两个文件。

`twitter` 是独立的上游命令，不会读取 FactReach 的配置文件。直接运行
...
如果你已经安装了 [bird CLI](https://www.npmjs.com/package/@steipete/bird)（`npm install -g @steipete/bird`），它也能正常工作。FactReach 会自动检测并使用已安装的 bird。两者功能类似，twitter-cli 是当前推荐方案。
