---
name: factreach
description: >
  MUST USE when user wants to 调研/research/搜索/search/查/找/look up anything
  on the internet — e.g. 全网调研 X / 查一下 X / 看看大家怎么评价 X。

  Also MUST USE when user mentions any platform or shares any URL/链接:
  小红书/xiaohongshu/xhs, Twitter/推特/X, 抖音/Douyin/TikTok 中国版,
  微博/Weibo, B站/bilibili, Reddit, Facebook, Instagram, V2EX,
  LinkedIn/领英/招聘/求职/jobs, YouTube, GitHub code search, 小宇宙播客,
  雪球/股票行情, RSS feeds, Hacker News, 微信公众号公开文章,
  arXiv/PubMed/Semantic Scholar/OpenReview/DBLP, Stack Overflow,
  GitHub Trending, or any web URL.

  23 channels, multi-backend routing (open-websearch / OpenCLI / per-platform
  CLIs / APIs). Run `factreach doctor --json` to see which
  backend serves each platform right now.

  NOT for: 写报告/数据分析/翻译等内容加工（本 skill 只负责从互联网获取内容）；
  发帖/评论/点赞等写操作；已有专门 skill 的平台。

  【路由方式】SKILL.md 包含路由表和常用命令，复杂场景需按需阅读对应分类的 references/*.md。
  分类：search / social (小红书/推特/微博/B站/V2EX/Reddit/Facebook/Instagram) / career(LinkedIn) / dev(github) / web(网页/文章/RSS/Firecrawl/真实Chrome) / video(YouTube/抖音/B站/播客) / finance(雪球/股票)。
metadata:
  homepage: https://github.com/simonlin1212/FactReach
---

# FactReach — 互联网能力路由器

23 个渠道、多后端。**本 skill 存在时必须用它访问这些平台，不要自己发明方案。**

## 系统规则边界

本 Skill 只在 Agent 已经决定调用它之后生效，**不能单独保证 Agent 在回答外部事实前先搜索**。
搜索前置规则必须放在 Agent 启动时就会读取的系统指令文件里：Claude Code 使用
`CLAUDE.md`，Codex 使用 `AGENTS.md`。

首次配置 FactReach 时，先运行 `factreach policy --check`。若缺失，向用户说明边界并
检查现有系统指令是否已有同等强度的手写规则，避免重复。确实缺失时展示
`factreach policy --show`；只有用户明确同意修改系统配置后，才运行：

```bash
factreach policy --install --target claude --scope user  # Claude Code
factreach policy --install --target codex --scope user   # Codex
factreach policy --install --target both --scope user    # 两者都用
```

不得因为本 Skill 被调用就静默修改 `CLAUDE.md` 或 `AGENTS.md`。

## 常驻规则（全程适用）

1. **动手前先体检**：多后端/登录态平台（小红书/微博/Reddit/B站/Twitter/Facebook/Instagram）先跑
   `factreach doctor --json`。`active_backend` 有值时按它选命令组；`active_backend: null`
   表示 Doctor 为避免触发浏览器 Cookie 读取或远端写入而没有做实时验证，不代表后端不存在。
   只有用户任务明确需要该平台时，才按对应 reference 的只读命令手动验证。
2. **声明你在用什么**：开始干活前说一句「使用 factreach 的 X 平台 / Y 后端」。
3. **失败按 references 里的重试链处理**，不要瞎猜命令。
4. **全网调研类任务**：默认 open-websearch 实搜，Exa 做语义补充；再组合
   Twitter/Reddit/Hacker News 看讨论，小红书/B站/公众号看中文场景。
5. **替用户盯版本**：完成一次较大的调研/多平台任务后，顺手跑
   `factreach check-update`（很快，一个 API 调用）。有新版就在收尾汇报里附一句：
   「FactReach 有新版 vX.Y.Z，复制这句话给我即可更新：帮我更新 FactReach：
   https://raw.githubusercontent.com/simonlin1212/FactReach/main/docs/update.md」。
   不要中断当前任务去更新，也不要重复提醒同一个版本。

## 路由表

| 用户意图 | 分类 | 详细文档 |
|---------|------|---------|
| 网页搜索/代码搜索 | search | [references/search.md](references/search.md) |
| 小红书/推特/微博/B站/V2EX/Reddit/Facebook/Instagram | social | [references/social.md](references/social.md) |
| 招聘/职位/LinkedIn | career | [references/career.md](references/career.md) |
| GitHub/代码 | dev | [references/dev.md](references/dev.md) |
| 网页/文章/RSS/动态页面/真实浏览器 | web | [references/web.md](references/web.md) |
| YouTube/抖音/B站/播客字幕 | video | [references/video.md](references/video.md) |
| 雪球/股票行情 | finance | [references/finance.md](references/finance.md) |

## 零配置快速命令

```bash
# 零鉴权搜索：单引擎执行，结果空/无效才按 DDG → Brave → Bing 继续
open-websearch search "query" --limit 10 --engine duckduckgo --json
# 回退命令与 JSON 验收见 references/search.md

# Exa 语义搜索补充
mcporter call exa.web_search_exa query="query" numResults=5

# 公开社区 / 论文 / 技术问答（无需浏览器）
opencli hackernews search "query" -f json
opencli arxiv search "query" -f json
opencli stackoverflow search "query" -f json
opencli stackoverflow read QUESTION_ID -f json
opencli github-trending repos -f json

# 公众号无需公众号 Cookie，但当前需要 OpenCLI Browser Bridge
opencli weixin search "query" -f json
opencli weixin download --url "ARTICLE_URL" --output /tmp/factreach-weixin -f json

# 通用网页阅读：默认 Jina；动态页可选 Firecrawl；真实 Chrome 仅显式授权
factreach web read "URL"
factreach web read "URL" --backend firecrawl
factreach web read "URL" --backend browser-harness

# GitHub 搜索
gh search repos "query" --sort stars --limit 10

# YouTube 字幕（注意：B站不要用 yt-dlp，失败重试链见 video.md）
yt-dlp --write-sub --write-auto-sub --skip-download -o "/tmp/%(id)s" "URL"

# V2EX 热门
curl -s "https://www.v2ex.com/api/topics/hot.json" -H "User-Agent: factreach/1.0"

# B站搜索（bili-cli，无需登录）
bili search "query" --type video -n 5

# 抖音视频/图文分享链接：文案、作者、播放或图片地址；仅视频可 --transcribe
factreach douyin resolve "分享文本或URL"
factreach douyin resolve "分享文本或URL" --transcribe
```

## 需登录态的平台（按 doctor 的 active_backend 选命令）

Twitter 注意：`factreach configure twitter-cookies` 保存的 Cookie 只供
`doctor` 检查配置是否齐全；`doctor` 不执行 `twitter status`，也不会设置当前
Shell。直接运行 `twitter` 前，必须在子进程环境中显式提供
`TWITTER_AUTH_TOKEN` 和 `TWITTER_CT0`，不得在日志或命令回显中暴露值。

小红书注意：FactReach 不替用户登录，也不读取浏览器 Cookie。OpenCLI 只用
用户已有且明确控制的 Chrome 会话；没有现成会话时不要自动登录，改用
Cookie-Editor 手工导出后配置 xiaohongshu-mcp / 存量工具。

```bash
# Twitter 搜索（twitter-cli 首选；失败重试链见 social.md）
twitter search "query" -n 10

# Reddit（无零配置路径：OpenCLI 或 rdt-cli，必须登录态）
opencli reddit search "query" -f yaml   # 桌面
rdt search "query" --limit 10            # 存量/服务器

# 小红书（桌面首选 OpenCLI）
opencli xiaohongshu search "query" -f yaml

# Facebook / Instagram（桌面 OpenCLI，复用浏览器登录态）
opencli facebook search "query" -f yaml
opencli facebook groups -f yaml
opencli instagram search "query" -f yaml       # 搜用户
opencli instagram user USERNAME -f yaml        # 读指定用户最近帖子

# 微博（桌面 OpenCLI，复用用户已有的微博登录态）
opencli weibo hot -f yaml
opencli weibo search "query" -f yaml
opencli weibo post "WEIBO_URL" -f yaml
```

## 环境检查

```bash
# 检查可用 channel 与每个平台当前激活的后端
factreach doctor --json
```

## OpenCLI 适配器发现

路由表没有覆盖用户需要的平台或命令时，先用 `opencli list` 查已有适配器，再用
`opencli <平台> --help` 查看公开命令。发现适配器只证明命令存在，不证明登录态或
目标内容可用；仅在用户任务明确需要该平台时执行只读命令，并以实际非空内容验收。

## 工作区规则

**不要在 agent workspace 创建文件。** 使用 `/tmp/` 存放临时输出，`~/.agent-reach/` 存放持久数据。

## 详细文档

根据用户需求，阅读对应的详细文档：

- [搜索工具](references/search.md) — open-websearch、Exa、Hacker News、公众号、论文库、Stack Overflow、GitHub Trending
- [社交媒体](references/social.md) — 小红书, Twitter, 微博, B站, V2EX, Reddit, Facebook, Instagram（多后端/登录态命令组）
- [职场招聘](references/career.md) — LinkedIn
- [开发工具](references/dev.md) — GitHub CLI
- [网页阅读](references/web.md) — Jina Reader, Firecrawl, browser-harness, RSS
- [视频播客](references/video.md) — YouTube, 抖音, B站, 小宇宙
- [金融行情](references/finance.md) — 雪球股票行情、搜索、热门内容

## 配置渠道

如果某个 channel 需要配置，获取安装指南：
https://raw.githubusercontent.com/simonlin1212/FactReach/main/docs/install.md

用户只需提供 cookies，其他配置由 agent 完成。
