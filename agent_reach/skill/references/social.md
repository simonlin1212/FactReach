...
小红书有三个后端，**先跑 `factreach doctor --json` 看 xiaohongshu 的 `active_backend` 是哪个**，再用对应命令组。
...
> 的 Chrome 会话；FactReach 不替用户登录，也不读取浏览器 Cookie。
> `factreach configure xhs-cookies` 不会把 Cookie 注入 OpenCLI。
...
> **认证边界**: FactReach 不得替用户执行小红书登录，也不得读取浏览器
...
`factreach configure twitter-cookies` 通过隐藏输入保存的 Cookie 只供
`factreach doctor` 检查显式凭据是否齐全。`doctor` 不执行上游
...
curl -s "https://www.v2ex.com/api/topics/show.json?node_name=python&page=1" -H "User-Agent: factreach/1.0"
```
...
curl -s "https://www.v2ex.com/api/topics/show.json?id=TOPIC_ID" -H "User-Agent: factreach/1.0"
```

### 主题回复

```bash
curl -s "https://www.v2ex.com/api/replies/show.json?topic_id=TOPIC_ID&page=1" -H "User-Agent: factreach/1.0"
```

### 用户信息

```bash
curl -s "https://www.v2ex.com/api/members/show.json?username=USERNAME" -H "User-Agent: factreach/1.0"
```
...
**Reddit 没有零配置路径**：匿名 `.json` 端点已被封（403），官方 API 自 2025-11 起人工审批基本不批。两个后端都靠登录态，先跑 `factreach doctor --json` 看 reddit 的 `active_backend`。中国大陆访问需代理。
...
Facebook 走 OpenCLI，复用用户 Chrome 里的 facebook.com 登录态。先跑 `factreach doctor --json` 看 facebook 的 `active_backend`，正常应为 `OpenCLI`。不要推荐 Jina/Exa/Graph API 作为默认路径。
...
Instagram 走 OpenCLI，复用用户 Chrome 里的 instagram.com 登录态。先跑 `factreach doctor --json` 看 instagram 的 `active_backend`，正常应为 `OpenCLI`。不要默认恢复 instaloader；历史上 cookies/401/429 不稳定。
...
## 微博 / Weibo（OpenCLI，必须登录态）

微博首选 OpenCLI，复用用户已经登录且明确控制的 Chrome 会话。`doctor` 只检查后端
是否具备，不读取微博页面；任务真正需要微博时，再执行下列只读命令并验收非空内容。
...
# 用户近期微博与评论
opencli weibo user-posts "USER_URL_OR_ID" -f yaml
opencli weibo comments "WEIBO_URL" -f yaml
```

OpenCLI 失败且页面必须登录或交互时，经用户明确同意后可用：
`factreach web read "WEIBO_URL" --backend browser-harness`。不要自动登录、不要保存
微博账号密码，也不要把空结果解释成“微博没有相关内容”。
