...
YouTube、抖音、B站、小宇宙播客的字幕和转录。

## 抖音 / Douyin（视频/图文单链接解析）

```bash
# 分享文本或短链接 → 内容类型、作品 ID、发布文案、作者、播放或图片地址
factreach douyin resolve "https://v.douyin.com/XXXX/"

# 同时提取视频里的口播文字（需要 ffmpeg + Groq/OpenAI）
factreach douyin resolve "https://v.douyin.com/XXXX/" --transcribe
```

解析发布文案、视频播放地址或图文 `image_urls` 无需登录、无需 Cookie。输出中的
`content_type` 会明确标为 `video` 或 `note`；平台返回的地址可能过期，需要保存内容时应在
当前任务内及时使用。`--transcribe` 只适用于视频，图文作品会明确报错；默认 `auto` 只用
第一个已配置服务商，不会无授权跨服务商降级。
...
4. 仍失败或视频本来就没有字幕：`factreach transcribe "URL"` 下载音频转写。
...
factreach transcribe "https://www.youtube.com/watch?v=VIDEO_ID"
factreach transcribe ./local_audio.mp3 -o /tmp/transcript.txt
```

> `factreach transcribe` 只接收公开 http(s) URL 或本地音频文件。用 `ytsearch5:` 搜索时，先从 yt-dlp 结果里选出具体视频 URL，再转写。
> 需要先配置 key：`factreach configure groq-key`（隐藏输入；免费，console.groq.com）
> 或 `factreach configure openai-key`。默认 auto 模式只使用第一个已配置服务商
...
# 下载音频并切分为 ASR-ready WAV（无字幕时配合 factreach transcribe 转写）
bili audio BVxxx
```
...
3. **配置 Key**: `factreach configure groq-key`（隐藏输入）
4. **首次运行**: `factreach install --env=auto --system --channels=xiaoyuzhou`（需用户明确授权）
...
| 场景 | 推荐工具 |
|-----|---------|
| YouTube 字幕 | yt-dlp；失败时 OpenCLI（最多 3 次）→ factreach transcribe |
| 抖音视频/图文文案与媒体地址 | factreach douyin resolve |
| 抖音口播文字 | factreach douyin resolve --transcribe |
| B站视频详情/搜索 | bili-cli |
| B站字幕 | opencli bilibili subtitle |
| 播客转录 | 小宇宙 transcribe.sh |
| 无字幕音视频 | factreach transcribe（B站音频先 `bili audio`） |
