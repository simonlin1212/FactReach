...
from agent_reach.probe import ProbeResult, probe_command
...
_MAX_CLI_MANIFEST_BYTES = 8 * 1024 * 1024
...
WECHAT_OPENCLI_ADAPTERS = ("weixin",)
WEIBO_OPENCLI_ADAPTERS = ("weibo",)
...
def probe_opencli_public(timeout: int = 10):
    """Probe the public OpenCLI runtime without requiring its browser bridge.

    OpenCLI has two materially different execution paths: browser-session
    adapters and public HTTP adapters.  The latter do not need Chrome, a
    logged-in session, or the local daemon.  Keep this probe separate so a
    disconnected extension never hides genuinely zero-auth capabilities.
    """
...
def _opencli_manifest_path() -> Path | None:
    """Locate OpenCLI's bundled command manifest without executing OpenCLI.

    npm installs normally expose ``opencli`` as a symlink into the package.
    Windows uses a sibling ``node_modules`` directory instead, so check both
    layouts. Reading this static file keeps ``factreach doctor`` side-effect
    free; invoking ``opencli list`` can initialise ``~/.opencli``.
    """
    ...
    for parent in (resolved.parent, *resolved.parents):
        candidates.append(parent / "cli-manifest.json")
        if parent.name == "opencli" and parent.parent.name == "@jackwener":
            break
    candidates.append(
        path.parent / "node_modules" / "@jackwener" / "opencli" / "cli-manifest.json"
    )
...
def _read_opencli_manifest() -> ProbeResult:
    manifest_path = _opencli_manifest_path()
    if manifest_path is None:
        return ProbeResult(
            "error",
            hint="OpenCLI 安装中找不到 cli-manifest.json，请重新安装官方 npm 包",
        )
    try:
        size = manifest_path.stat().st_size
        if size <= 0 or size > _MAX_CLI_MANIFEST_BYTES:
            return ProbeResult("error", hint="OpenCLI 适配器清单大小异常")
        raw = manifest_path.read_bytes()
    except OSError as exc:
        return ProbeResult("error", hint=f"OpenCLI 适配器清单无法读取：{exc}")
    try:
        raw_text = raw.decode("utf-8")
    except UnicodeDecodeError:
        return ProbeResult("error", hint="OpenCLI 适配器清单不是 UTF-8")
    return ProbeResult("ok", output=raw_text, stdout=raw_text)


def probe_opencli_adapters(required: tuple[str, ...], timeout: int = 10):
    """Verify that the installed OpenCLI contains every requested adapter."""
    runtime = probe_opencli_public(timeout)
    if not runtime.ok:
        return runtime

    # Do not probe with ``opencli <adapter> --help``: OpenCLI also exits 0
    # and prints its top-level help for an unknown adapter. The package's
    # structured manifest is authoritative and can be read without allowing
    # OpenCLI startup to initialise files under the user's home directory.
    manifest = _read_opencli_manifest()
    if not manifest.ok:
        if not manifest.hint:
            manifest.hint = "OpenCLI 无法读取公开适配器清单"
        return manifest
    try:
        # Structured output is stdout-only. OpenCLI may append an update notice
        # to stderr after a successful command; parsing the combined diagnostic
        # stream would reject an otherwise healthy manifest.
        entries = json.loads(manifest.stdout or manifest.output)
    except json.JSONDecodeError:
        return ProbeResult("error", hint="OpenCLI 适配器清单不是有效 JSON")
    if not isinstance(entries, list):
        return ProbeResult("error", hint="OpenCLI 适配器清单结构无效")
    ...
    missing = [adapter for adapter in required if adapter not in sites]
    if missing:
        return ProbeResult(
            "error",
            hint=f"OpenCLI 缺少公开适配器：{', '.join(missing)}",
        )
    return runtime


def probe_opencli_public_adapters(timeout: int = 10):
    """Verify that the installed OpenCLI contains every public adapter we expose."""
    return probe_opencli_adapters(PUBLIC_OPENCLI_ADAPTERS, timeout=timeout)


def probe_opencli_wechat_adapter(timeout: int = 10):
    """Verify that the installed OpenCLI contains the WeChat adapter."""
    return probe_opencli_adapters(WECHAT_OPENCLI_ADAPTERS, timeout=timeout)


def probe_opencli_weibo_adapter(timeout: int = 10):
    """Verify that the installed OpenCLI contains the Weibo adapter."""
    return probe_opencli_adapters(WEIBO_OPENCLI_ADAPTERS, timeout=timeout)
