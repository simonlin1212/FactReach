...
**症状：** `factreach doctor` 显示雪球 ⚠️，报 `HTTP Error 400`
...
再次运行 `factreach doctor` 确认恢复 ✅。Cookie 过期后重新运行即可。
...
环境变量才能访问 Twitter API。`factreach configure twitter-cookies`
...
>
> **Fallback：** 如果你已经安装了 bird CLI（`npm install -g @steipete/bird`），它也能正常工作。FactReach 会自动检测已安装的工具。
