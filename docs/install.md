...
> 🛡️ **Safe by default:** `factreach install` only checks the machine and lists missing dependencies:
> ```
> 帮我安全检查并安装 FactReach：https://raw.githubusercontent.com/simonlin1212/FactReach/main/docs/install.md
> ```
...
Install FactReach and all upstream tools so your user's agent has full internet access. After installation, you'll use the upstream tools directly (OpenCLI, twitter-cli, bili-cli, rdt-cli, yt-dlp, mcporter, gh CLI, etc.) — FactReach is the selector, installer, health checker and router, never a wrapper.
...
All FactReach files go in dedicated directories — **never in the agent workspace**:
...
| Skills | `~/.openclaw/skills/factreach/` | SKILL.md |
| Evidence policy (opt-in) | `~/.claude/CLAUDE.md` / `~/.codex/AGENTS.md` | Managed markdown block |
...
The evidence policy is a separate, explicit action because it changes the
agent's startup instructions. Preview it with `factreach policy --show`; only
after the user approves, install it with `factreach policy --install
--target claude|codex|both --scope user|project`. The normal installer and
Skill registration never write `CLAUDE.md` or `AGENTS.md` silently.
Remove only the managed block with `factreach policy --uninstall` and the
same explicit target/scope.

### Step 1: Install the basics

If Agent-Reach is already installed in the same Python environment, remove its
distribution first. FactReach intentionally keeps the `agent_reach` Python
package and legacy command for compatibility, so installing both distributions
side by side would make them own the same files:

```bash
# If the old distribution was installed with pipx:
pipx uninstall agent-reach

# If it was installed with pip in this Python environment:
python -m pip uninstall agent-reach
```

This command removes the Python distribution only. It does not delete the
shared `~/.agent-reach/` configuration, cookies, tokens, or tools.

```bash
# 推荐：pipx（最省心）
pipx install https://github.com/simonlin1212/FactReach/archive/main.zip
factreach install --env=auto               # Read-only check (default)
...
> python -m pip install https://github.com/simonlin1212/FactReach/archive/main.zip
...
- Web (Jina Reader), Douyin link parsing, YouTube, GitHub, RSS, Exa Search, V2EX, Bilibili (basic)

> 💡 **macOS / Homebrew Python 提示 `externally-managed-environment`？**
> 这是 PEP 668 保护，不是 FactReach 本身的问题。优先用 `pipx install ...`，或先创建 `venv` 再安装。

**Install modes:**

```bash
factreach install --env=auto             # Check only; safe default
factreach install --env=auto --safe      # Same check-only behavior (compatibility)
factreach install --env=auto --system    # Explicitly allow external/system installs
factreach install --env=auto --dry-run   # Preview what --system would do
```
...
> 还有这些可选渠道，你需要哪些？
>
> - 🔍 **open-websearch** — DuckDuckGo / Brave / Bing 零鉴权全网搜索，Agent 按非空 JSON 顺序回退
> - 🧩 **公开信息源组** — Hacker News、arXiv、PubMed、OpenReview、DBLP、Semantic Scholar、Stack Overflow、GitHub Trending；无需 Cookie / Chrome 扩展（Semantic Scholar 匿名额度可能 429）
> - 📰 **公众号公开文章** — 不需公众号 Cookie，但最新版实跑仍需 OpenCLI Browser Bridge
> - 🌟 **OpenCLI**（桌面推荐）— 一次安装即可提供 Reddit/Facebook/Instagram/B站字幕/Twitter 备选，并作为小红书桌面后端；小红书只使用用户已有且明确控制的 Chrome 会话
> - 🎵 **抖音** — 视频/图文分享链接、发布文案、作者、播放或图片地址解析内置；视频口播转写另需 Groq/OpenAI
> - 🧣 **微博** — 热搜、搜索、博文、用户内容、评论（桌面 OpenCLI，复用 Chrome 登录态）
> - 🔥 **Firecrawl** — 动态网页抓取（托管 Key 或自托管 API；可能消耗额度）
> - 🌐 **browser-harness** — 真实 Chrome 登录/交互页读取；只在用户显式选择时连接
...
```bash
factreach install --env=auto --system --channels=opencli,xiaohongshu   # Desktop user chose XHS
factreach install --env=auto --system --channels=open-websearch,hackernews,wechat,academic,stackoverflow,github-trending
factreach install --env=auto --system --channels=facebook,instagram    # Desktop Meta channels
factreach install --env=auto --system --channels=douyin,weibo,firecrawl,browser-harness
factreach install --env=auto --system --channels=all                   # User approved everything
```

Supported channel names: `opencli`, `open-websearch`, `hackernews`, `wechat`, `academic`, `stackoverflow`, `github-trending`, `twitter`, `xiaoyuzhou`, `xueqiu`, `xiaohongshu`, `reddit`, `facebook`, `instagram`, `bilibili`, `linkedin`, `douyin`, `weibo`, `firecrawl`, `browser-harness`, `all`

### Step 3: Fix what's broken

Run `factreach doctor` and check the output.
...
> 4. 把导出的字符串发给 Agent
>
> Twitter 只接受用户通过 Cookie-Editor 明确导出的内容。FactReach 不替用户执行小红书登录，也不读取小红书浏览器 Cookie；小红书 OpenCLI 只使用用户已有且明确控制的 Chrome 会话。没有现成会话时，改用 Cookie-Editor 导出后配置 xiaohongshu-mcp / 存量工具。雪球、Bilibili 可按平台显式导入，例如 `factreach configure --from-browser chrome --platform xueqiu`；命令不会扫描或保存其他平台。
...
这会把 `twitter_auth_token` 和 `twitter_ct0` 保存给 FactReach 自己的
...
>
> **你（Agent）需要做的：**
> 1. 确认用户配了代理：`factreach configure proxy`（隐藏输入）
> 2. 设置环境变量：`export HTTP_PROXY="..." HTTPS_PROXY="..."`
> 3. FactReach 会自动处理剩下的，不需要用户做额外操作
>
...
> ```bash
> factreach configure proxy
> ```

**XiaoHongShu / 小红书（多后端，按环境选）:**

> **认证边界：** FactReach 不替用户执行小红书登录，也不读取浏览器
> Cookie。OpenCLI 只使用用户已经存在且明确控制的 Chrome 会话；
> `factreach configure xhs-cookies` 不会把 Cookie 注入 OpenCLI 或 Chrome。
> 如果没有现成会话，不要自动登录；改用 Cookie-Editor 手工导出后配置
> xiaohongshu-mcp 或存量工具：
>
> ```bash
> factreach configure xhs-cookies
> ```
>
...
```bash
factreach install --system --channels opencli
```
...
```bash
factreach install --system --channels facebook,instagram
```
...
```bash
factreach configure --from-browser chrome --platform xueqiu
```
...
脚本已随 FactReach 自动安装，用户只需提供 Key：

```bash
factreach configure groq-key
```
...
Run `factreach doctor` one final time and report the results to your user.

### Step 5: Set up daily monitoring (OpenClaw only)

If you are running inside **OpenClaw**, ask your user:

> "FactReach 安装好了。要不要我设一个每天自动检查的任务？它会帮你盯着这些渠道是否正常、有没有新版本。有问题才会通知你，没问题不打扰。"

If the user agrees, create a **cron job** (daily, `sessionTarget: "isolated"`, `delivery: "announce"`) with this task:

```
运行 factreach watch 命令。
如果输出包含"全部正常"，不需要通知用户，静默结束。
如果输出包含问题（❌ ⚠️）或新版本（🆕），把完整报告发给用户，并建议修复方案。
如果有新版本可用，问用户是否要升级（把这句话发给用户的 Agent 即可完整更新：帮我更新 FactReach：https://raw.githubusercontent.com/simonlin1212/FactReach/main/docs/update.md）。
```
...
|---------|-------------|
| `factreach install --env=auto` | Read-only dependency and channel check (default) |
| `factreach install --env=auto --system` | Explicitly install/configure core external tools |
| `factreach install --env=auto --system --channels=twitter,xiaohongshu` | Install approved optional channels |
| `factreach install --env=auto --system --channels=all` | Install everything after explicit approval |
| `factreach install --env=auto --safe` | Compatibility alias for the safe default |
| `factreach install --env=auto --dry-run` | Preview what would be done |
| `factreach doctor` | Show channel status |
| `factreach watch` | Quick health + update check (for scheduled tasks) |
| `factreach check-update` | Check for new versions |
| `factreach configure twitter-cookies` | 通过隐藏输入保存 Twitter Cookie；直接调用仍需显式环境变量 |
| `factreach configure proxy` | 通过隐藏输入保存代理地址；不是自动解锁开关 |
| `factreach configure groq-key` | 通过隐藏输入配置小宇宙转录 Key |
| `factreach configure firecrawl-key` | 通过隐藏输入配置 Firecrawl 托管 Key |
| `factreach configure firecrawl-url URL` | 配置 Firecrawl 自托管 `/v2` 地址 |
| `factreach douyin resolve "URL"` | 解析抖音视频/图文文案、作者与临时播放或图片地址 |
| `factreach web read URL` | Jina 优先读取网页；已配置时可回退 Firecrawl |
| `factreach web read URL --backend browser-harness` | 经显式授权，用真实 Chrome 读取页面 |
...
| Douyin | `factreach` | `factreach douyin resolve "SHARE_URL"` |
| Weibo | `opencli` | `opencli weibo search "query" -f yaml` |
| GitHub | `gh` | `gh search repos "query"` |
| Web | `factreach` | `factreach web read "URL"`（Jina → Firecrawl；真实 Chrome 需显式选择） |
| Exa Search | `mcporter` | `mcporter call exa.web_search_exa query="..." numResults=5` |
| Web Search | `open-websearch` | 先 `--engine duckduckgo` 并验收非空 JSON；失败再依次换 `brave` / `bing` |
| Hacker News | `opencli` public | `opencli hackernews search "query" -f json` |
| 微信公众号公开文章 | `opencli` Browser Bridge | `opencli weixin search "query" -f json` / `opencli weixin download --url URL --output /tmp/factreach-weixin -f json` |
| 学术论文 | `opencli` public | `opencli arxiv search "query" -f json`（同组还有 pubmed / semanticscholar / openreview / dblp；Semantic Scholar 匿名额度可 429） |
| Stack Overflow | `opencli` public | `opencli stackoverflow search "query" -f json` / `opencli stackoverflow read QUESTION_ID -f json` |
| GitHub Trending | `opencli` public | `opencli github-trending repos -f json` |
...
> 多后端平台以 `factreach doctor --json` 的 `active_backend` 为准。
